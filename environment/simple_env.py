...
def compute_distance(target_point, query_point):
    """
    A function to compute distance between target metirc and query points
    
    Args:
    target_point (1 x 3 tensor)
    query_point (1 x 3 tensor)
    
    Returns:
    distance : Euclidean distance between current point and target point 
    """
    
    distance = torch.norm(target_point - query_point)
...
def center_of_mass(tensor):
    ...
    com = [torch.sum(indices[dim] * tensor) / total_mass for dim in range(tensor.dim())]
...
class CsvWriter:
    """
    A class that creates csv file writers 
    
    For each time step, saves the lesion idx, x,y,z grid pos (in img coords)
    Saves patient id and lesion idx sampled 
    
    """
    
    def __init__(self, csv_path, headers = None):
        """
        Saves path name and initialises a csv file with the following headers provided 
        """
        
        self.csv_path = csv_path # path for csv file
        
        if headers == None:
            headers = ["patient_id", "simlesion_id", "lesion_id", "x_img", "y_img", "z_img", "reward", "leson_x", "lesion_y", "lesion_z"]
    ...
    def write_step(self, 
                   ...
                   reward : int,
                   lesion_pos : torch.Tensor):
        
        # Breaks down grid pos into x,y,z
        
        # Break down lesion pos into x y z 
        
        # save into csv file 
        patient_id = patient_ids['patient_id']
        simpatient_id = patient_ids['simpatient_id']
        lesion_id = patient_ids['lesion_id']
        
        all_data = [patient_id, simpatient_id, lesion_id, x,y,z, reward, x_l, y_l, z_l]

        #with open(self.csv_path, mode = 'a') as file:
        
        np.savetxt(self.csv_path, all_data, delimiter = ',')
        
    def write_newpatient(self):
        """
        A file that adds blank row to csv file 
        """
        
        with open(self.csv_path, 'a') as file:
            file.write("\n") # new row for next patient
            
        
        
class VisualiseResults:
    """
    A class to visualise results from agents 
    """
    ...
    @staticmethod 
    def visualise_step(obs, grid_pos):

        # Obtain current pos (grid_pos)
        
        
        # Obtain US pos (overlaid with images)
        
        # Overlay needle pos 
        num_slices = obs.size()[0]
        
        fig, axs = plt.subplots(1,num_slices)
        
        for i in range(num_slices):
            axs[i].imshow(obs[i,:,:].numpy())
            axs[i].axis('off')
            
        plt.savefig("obs_visualise.png")        
        print('fuecoco')
        pass 
    
    @staticmethod 
    def visualise_all_steps(whole_vols, all_obs, grid_pos):
        """
        Parameters:
        
        whole_vols: torch.tensor
        all_obs : llist of torch.tensor
        grid_pos : list of grid positions used for targeting 
        """
        
        # Obtain entire vol (lesion, gland, obs)
        
        # Overlay grid with targeted lesion positions 
...
class TargettingEnv(gym.Env):
    ...
    def __init__(self, 
        ...
        self.lesion_hit = 0 
        self.csvwriter = CsvWriter(csv_path)
        ...
        self.min_hits = min_hits # how many needle hits before terminates (for hit reward)
        self.deform = deform # whether to deform or not 
        self.change_voxdims = change_voxdims 
        self.inc_shaped = inc_shaped # whether to include shaped reward or not!
        self.debug = debug # whether to give blank observations (to test if US images are helping or not!)
        self.inc_penalty = inc_penalty  # whether to penalise for choosing same grid position as before 
    ...
    def step(self, actions = torch.tensor([1,1,1])):
        ...
        #print(f"After deformation update :Target unique vals : {torch.unique(self.world.target)}")
        obs = self.observation.update(self.world, sample, self.step_counter).to(self.device)
    
        if self.debug:
            # print(f"Debugging obs")
            obs = self.debug_obs(obs)
            
        #print(f"After obs update : Target unique vals : {torch.unique(self.world.target)}")
        
        #  4. Compute CCL metrics, given x,y and depth 
        ccl = self.compute_ccl(obs, test_coords)
        ccl_v2 = 0 
        ccl_v3, hr = self.compute_CCL_HR(self.grid_pos, depth = sample)
        print(f'CCL : {ccl} cCL_v3 : {ccl_v3}')
        ...
        if self.reward_fn == 'needle':
            ...
            needle_reward = self.compute_reward_needle_v2(hr, ccl, ccl_v2, ccl_v3)

        if needle_reward > 0:
            self.lesion_hit += 1 
        
        # Give either obs or needle reward fn
        if (self.reward_fn == 'needle') or (self.reward_fn == 'needle_v2'):
        ...
        if self.inc_penalty:
            same_pos = self.check_same_pos(self.prev_pos_mm,self.world.observe_mm) 
            
            if same_pos:
                reward = -0.5 # max negative reward is -1, so negative reward is -0.5 instead of -01
            
        # Add shaped reward if included 
        if self.inc_shaped: 
            
            if not (self.reward_fn == 'obs'):# needle or needle_v2 
                magnitude = 1 # +1 -1 for signed reward -> from -1 -> (0 for missed, but closer to target vs -2 for further and misisng target)
            else: 
                magnitude = 0.1 # similarly moves -0.1 to 0 for missed, but closer to target vs -0.2 for further and missing target! 
            
            shaped_reward = self.compute_shaped_reward(self.target_centroid, self.prev_pos_mm, self.world.observe_mm, magnitude = magnitude)

            #print(f"Shaped reward included! before reward : {reward} after : {reward + shaped_reward}")
            reward += shaped_reward
            
        #print(f"Step : {self.step_counter-1}  CCL sampled : {ccl_v3} Reward : {reward}")
        ...
        lesion_hit = (self.lesion_hit >= (self.min_hits)) # if hit at least 2 times (found)
        
        if self.reward_fn == 'needle':
            # needle reward 
            terminated = exceeds_step or lesion_hit 
        if self.reward_fn == 'needle_v2':
            # needle reward 
            terminated = exceeds_step or lesion_hit 
        else:
            # observation reward 
        ...
        info = {'num_steps' : self.step_counter, 
                ...
                'ccl_v3' : ccl_v3,
                'hr' : hr, 
                'num_obs' : self.slices_count,
                'num_hit' : self.lesion_hit}
        
        self.csvwriter.write_step(self.patient_ids, 
                                  self.grid_pos,
                                  reward,
    ...
    def check_same_pos(self, current_pos, prev_pos):
        """
        Checks if agent does not move (same pos as prev pos)
        """
        
        print(f"Chicken")
        if torch.all(current_pos == prev_pos):
            return True 
        else:
            return False 
    ...
    def get_gland(self):
        """
        Returns gland and img and mm centroid coords for visualisation purposes 
        """
        # Obtains centroid of gland 
        img_coords = torch.nonzero(self.world.gland.squeeze(), as_tuple=True)
        
        return self.world.gt_gland, img_coords, self.prostate_centroid
    
    def get_world_size(self):
        return self.world.vol_size
    
    def get_target(self, gt = False):
        """
        Returns target and computes centroid 
        """
        if gt: 
            target = self.world.gt_target 
        else:
    ...
    def update_pos(self, raw_actions, z_depth):
        ...
        #print(f"Actions {raw_actions} mapped actions : {actions}")
    ...
    def compute_CCL_HR(self, grid_pos, depth):
        ...
        # Compute with respect to ground truth target, not deformed 
        intersect = needle_vol * self.world.gt_target.squeeze() 
        z_intersect = torch.nonzero(intersect)[:,0]
        
        
        
        lesion = np.max(self.world.gt_target.squeeze().cpu().numpy(), 0)
        gland_proj = np.max(self.world.gt_gland.squeeze().cpu().numpy(), 0)
        needle = np.max(needle_vol.cpu().numpy(), axis = 0)
        us = self.world.us.squeeze()
        mr = self.world.mr.squeeze()
        gland = self.world.gt_gland.squeeze()
        
        # plt.figure()
        # plt.imshow(us.cpu()[80,:,:])
        # plt.savefig("TEST_MR.png")
        # plt.close()

        # plt.figure()
        # plt.imshow(lesion*5 + needle*2 + gland_proj)
        # plt.colorbar()
        # plt.savefig("TEST_LESION.png")
        # plt.close()
    ...
    def compute_ccl(self, obs, needle_coords):
        ...
        needle_sampled = sampler(
            self.world.gt_target.type(torch.float32), needle_sampled_coords_norm)
    ...
    def compute_reward(self, obs, inc_shaped = False, inc_needle = False, hr = None):
        ...
        if contains_lesion:
            REWARD += 1 # xchanged from 10 to 1 
        else:
    ...
    def compute_reward_needle(self, hr, ccl_1, ccl_2, ccl_3, magnitude = 10):
    ...
    def compute_reward_needle_v2(self, hr, ccl_1, ccl_2, ccl_3, magnitude = 10):
        
        """
        Compute reward based on whether HR / CCL is hit! 
        Assumes positive reward is x10 less likely to occur than negaitve award; deals with this?
        
        Rabnge of rewards:
        +2 for firing 2 and ending episode instantly
        0.2 for firing 2, but taking 20 steps to find it
        -0.1 for firing 1 but taking 20 steps to find it (19 negative -0.1)
        -2 for missing all 
        ...
        if lesion_hit or ccl_nonzero:
            reward =  magnitude  # 1 magnitude # changed from magnitude of 1 -> 10
        
        else:
            reward = -1 # change from -0.1 to -1 for greater urgency!!! 
            
        return reward 

    def compute_shaped_reward(self, target_pos, prev_pos, current_pos, magnitude = 1):
        """
        Computes a shaped reward with +1 for getting closer to point, -1 otherwise
        ...
        dist_prev = compute_distance(target_pos, prev_pos)
        dist_new = compute_distance(target_pos, current_pos)
        
        shaped_reward = magnitude*torch.sign(dist_prev - dist_new) # if dist_prev - dist_new is +ve (ie smaller distance now),.
        
        return shaped_reward.item()
    ...
    def debug_obs(self, obs):
        """
        Performs a debug test to check whether blank observations of US slices assists with learning or hinders with learning 
        """

        # Replace debug obs with obs 
        blank_us = torch.zeros_like(obs)
        blank_us[:,2:,:,:] = obs[:,2:,:,:]
    ...
    def initialise_world(self):
        ...
        #print(f"Target unique vals : {torch.unique(self.world.target)} ")
        ...
        if self.deform:
            self.transition.update(self.world, self.action)
            #print(f"After deformation : target unique vals : {torch.unique(self.world.target)} ")
        ...
        if self.debug: 
            initial_obs = self.debug_obs(initial_obs)
